...
describe Hand do
  ...
  describe "#drop_cards(idx)" do
    it "drops the card user selects" do
      hand.drop_cards(1)
      expect(hand.cards.count).to be(2)
    end

    it "drops the correct card" do
      hand.drop_cards(1)
      expect(hand.cards).to_not include(hand.cards[1])
    end
  end
  ...
  describe "#current" do
    describe "what is in the current hand" do
      it "should be an empty array when initialized" do
        expect(hand.current).to be([])
      end

      it "should contain correct num of cards even after receiving new cards" do
        current_hand = some_cards
        current_hand.receive_cards([Card.new(:KING, :hearts)])
        expect(current_hand.count).to be(4)
      end
    end
